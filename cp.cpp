...
long long int check(long long int mid,vector<int>& coins){
        long long int cnt=0;
        int n=coins.size();
        for(int i=0;i<n;i++){
            cnt+=mid/coins[i];
        ...
        for(int i=0;i<n;i++){
            for(int j=i+1;j<n;j++){
                int ext=coins[i]*coins[j];
                ext/=__gcd(coins[i],coins[j]);
                cnt-=mid/ext;
            }
        }
        // for(int )
